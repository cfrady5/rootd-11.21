...
const brand = {
  ...
  tint: '#eef2e4',
  tintAlt: '#dfe5cb',
  shadowSoft: 'rgba(76, 89, 55, 0.15)',
  shadowStrong: 'rgba(76, 89, 55, 0.2)'
};
      ...
      {Icon && (
        <motion.div
          ...
          style={{
            ...
            justifyContent: 'center',
            marginBottom: '24px',
            boxShadow: `0 4px 12px ${brand.shadowSoft}`
          }}
        >
          <Icon style={{ width: '36px', height: '36px', color: brand.primary }} />
        </motion.div>
      )}
      <h3 style={{
        fontSize: '20px',
      ...
      <div style={{ display: 'flex', gap: '8px' }}>
        ...
        {pages.map((page) => (
          <motion.button
            ...
            style={{
              ...buttonStyle,
              backgroundColor: page === currentPage ? brand.primary : 'white',
              color: page === currentPage ? 'white' : '#374151',
              borderColor: page === currentPage ? brand.primary : '#e5e7eb'
            }}
            whileHover={{ backgroundColor: page === currentPage ? brand.dark : '#f9fafb' }}
            whileTap={{ scale: 0.95 }}
          >
            {page}
          </motion.button>
...
export function StatusBadge({ status }) {
  ...
  pass: { bg: brand.tint, color: brand.primary, label: 'Pass' },
    ...
    done: { bg: brand.tint, color: brand.primary, label: 'Done' }
  };
...
export function StatCard({ label, value, change, trend, icon: Icon, loading, helper, onClick }) {
  if (loading) {
    return (
      <div style={{
        backgroundColor: 'white',
    ...
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div style={{ flex: 1 }}>
          ...
          {formattedChange && (
            <p style={{
              fontSize: '14px',
              fontWeight: '600',
              color: trend === 'up' ? brand.primary : trend === 'down' ? '#dc2626' : '#6b7280',
              margin: 0
            }}>
              {formattedChange}
            </p>
        ...
        {Icon && (
          ...
          >
            <Icon style={{ width: '28px', height: '28px', color: brand.primary }} />
          </motion.div>
        )}
      </div>
    </motion.div>
...
// Section Card - Reusable panel with premium variants
export function SectionCard({ title, description, action, children, minHeight, variant = 'default', tone = 'mint' }) {
  const toneTokens = {
    mint: {
      background: `linear-gradient(135deg, ${brand.tint} 0%, ${brand.tintAlt} 100%)`,
      border: '#cfd6b4',
      heading: brand.dark,
      body: brand.primary,
      shadow: `0 25px 60px ${brand.shadowStrong}`
    },
    sky: {
    ...
    amber: {
      ...
      heading: '#78350f',
      body: '#b45309',
      shadow: '0 25px 60px rgba(217, 119, 6, 0.2)'
    },
    lavender: {
      ...
      heading: '#5b21b6',
      body: '#7c3aed',
      shadow: '0 25px 60px rgba(124, 58, 237, 0.18)'
    }
  };
  ...
  const variantStyles = {
    ...
    tinted: {
      background: toneTokens[tone]?.background ?? toneTokens.mint.background,
      border: `1px solid ${toneTokens[tone]?.border ?? toneTokens.mint.border}`,
      boxShadow: toneTokens[tone]?.shadow ?? toneTokens.mint.shadow,
      heading: toneTokens[tone]?.heading ?? toneTokens.mint.heading,
      body: toneTokens[tone]?.body ?? toneTokens.mint.body
    }
  };
      ...
      {(title || action || description) && (
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '16px', flexWrap: 'wrap' }}>
          <div>
            ...
            {description && (
              <p style={{ margin: '6px 0 0', color: activeVariant.body, fontSize: '14px', maxWidth: '520px' }}>{description}</p>
            )}
          </div>
